...
class AclController extends Controller
{
    ...
    private function savePermissionToDatabase()
    {
        ...
        if($manager->addAcl($params) !== false){
            return responseJson('保存成功!',true,R('common', 'foundation.acl.index'));
        };
    ...
    private function updatePermissionToDatabase()
    {
        ...
        if(!$data) return responseJson(Lang::get('common.info_incomplete'));
        $params = new \App\Services\Admin\Acl\Param\AclSave();
        $params->setAttributes($data);
        $manager = new AclActionProcess();
    ...
    public function sort()
    {
        ...
        if( ! $sort or ! is_array($sort)) return responseJson(Lang::get('common.choose_checked'));
