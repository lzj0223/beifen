<?php
if( ! function_exists('upoadsImg'))
{
    function upoadsImg(\Symfony\Component\HttpFoundation\File\File $file, $savePath='', $basePath='/uploads')
    {
        $clientName = $file->getClientOriginalName();
        //所以这里道出了文件上传的原理,将文件上传的某个临时目录中,然后使用Php的函数将文件移动到指定的文件夹.
        $extension = $file->getClientOriginalExtension(); //上传文件的后缀.
        if(!isImage($extension)){
            return ['status'=>false,'msg'=>'请上传合法的图片！'];
        }
        $newName = md5(date('ymdhis').$clientName).".".$extension;
        $path = $savePath ? $basePath.'/'.$savePath.'/' : $basePath.'/'.date('ym').'/';
        try{
            $file -> move('.'.$path,$newName);
        }catch (Exception $e){
            return ['status'=>false,'msg'=>'图片上传失败啦！'.$e->getMessage()];
        }
        return ['status'=>true,'msg'=>'图片上传成功！','path'=>$path.$newName];
    }
}
 ...
 * @param $content 内容
 * @param string $color 关键词标亮的颜色
 * @return mixed
 */
function highLight($keywords,$content,$color='red') {
    arsort($keywords);
    foreach($keywords as $key=>$value){
        $placeholder[$key] = "<!--{".$key."}-->";
        $replace_keywords[$key] = "<span style='color: {$color}'>{$value}</span>";
    }

    $content = str_replace($keywords, $placeholder, $content);
    $content = str_replace($placeholder,$replace_keywords,$content);
...
{
    function responseJson($msg, $result = false,$redict='')
    {
